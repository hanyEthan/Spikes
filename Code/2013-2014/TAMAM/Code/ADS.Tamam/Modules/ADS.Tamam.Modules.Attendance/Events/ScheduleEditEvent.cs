...
{
    [Serializable]
    public class ScheduleEditEvent : EventCell, IXIncludable
    {
        #region Properties
        ...
        public ScheduleEditEvent( Guid scheduleId , List<Department> oldScheduleDepartments , List<Person> oldSchedulePersonnel ) : this()
        {
        ...
        public override bool Process()
        {
            try
            {
                ...
                var newScheduleDepartments = ScheduleDepartmentsResponse.Result.Select( x => x.Department );
                var allDepartments = OldScheduleDepartments.Union( newScheduleDepartments ).Distinct().ToList();
                ...
                var SchedulePeopleResponse = dataHandler.GetSchedulePersonnel_ScheduleId( ScheduleId , SystemSecurityContext.Instance );
                ...
                var newSchedulePeople = SchedulePeopleResponse.Result.Select( x => x.Person );
                var allPeople = OldSchedulePersonnel.Union( newSchedulePeople ).Distinct().ToList();
        ...
        {
            return ScheduleId != Guid.Empty;// && OldScheduleDepartments != null && OldSchedulePersonnel != null;
        }
