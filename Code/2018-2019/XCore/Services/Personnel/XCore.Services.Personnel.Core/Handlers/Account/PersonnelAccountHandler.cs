...
namespace XCore.Services.Personnel.Core.Handlers.Accounts
{
    ...
    public class PersonnelAccountHandler : IPersonnelAccountHandler
    {
        #region props.
        ...
        private readonly IModelValidator<PersonnelAccount> PersonnelAccountValidator;
        private readonly IPersonnelDataUnity dataHandler;
        #endregion
        #region cst.
        public PersonnelAccountHandler(IPersonnelDataUnity DataHandler, IModelValidator<PersonnelAccount> PersonnelAccountValidator)
        {
            this.dataHandler = DataHandler;
            this.PersonnelAccountValidator = PersonnelAccountValidator;
        ...
        public string ServiceId { get { return $"XCore.PersonnelAccountHandler.{Guid.NewGuid()}"; } }

        #endregion

        #region IAccountHandler
        public async Task<ExecutionResponse<SearchResults<PersonnelAccount>>> Get(PersonnelAccountSearchCriteria criteria, RequestContext requestContext)
        {
            try
            {
                var context = new ExecutionContext<SearchResults<PersonnelAccount>>();
                await context.Process(async () =>
                {
                    ...
                    var AccountBases = await dataHandler.PersonnelAccount.GetAsync(criteria, GetIncludes(criteria.SearchIncludes));
        ...
        public async Task<ExecutionResponse<PersonnelAccount>> Create(PersonnelAccount Account, RequestContext requestContext)
        {
            try
            {
                var context = new ExecutionContext<PersonnelAccount>();
                await context.Process(async () =>
                {
                    #region Logic
                    ...
                    await dataHandler.PersonnelAccount.CreateAsync(Account);
                ...
                {
                    Request = requestContext,
                    Validation = new ValidationContext<PersonnelAccount>(this.PersonnelAccountValidator, Account, ValidationMode.Create),
                });
                return context.Response;
        ...
        public async Task<ExecutionResponse<PersonnelAccount>> Edit(PersonnelAccount Account, RequestContext requestContext)
        {
            try
            {
                var context = new ExecutionContext<PersonnelAccount>();
                await context.Process(async () =>
                {
                    ...
                    var existing = await dataHandler.PersonnelAccount.GetFirstAsync(x => x.Id == Account.Id || x.Code == Account.Code, null, this.Account_DataInclude_Full);
                    MapUpdate(existing, Account, out List<int> addedSettings, out List<int> removedSettings);

                    dataHandler.PersonnelAccount.Update(existing);
                    await dataHandler.SaveAsync();
                ...
                {
                    Request = requestContext,
                    Validation = new ValidationContext<PersonnelAccount>(this.PersonnelAccountValidator, Account, ValidationMode.Edit),
                });
                return context.Response;
        ...
        public async Task<ExecutionResponse<bool>> Delete(PersonnelAccount Account, RequestContext requestContext)
        {
            try
            {
                ...
                await context.Process(async () =>
                {
                    ...
                    await dataHandler.PersonnelAccount.DeleteAsync(Account);
                    await dataHandler.SaveAsync();
                ...
                {
                    Request = requestContext,
                    Validation = new ValidationContext<PersonnelAccount>(this.PersonnelAccountValidator, Account, ValidationMode.Delete),
                });
                return context.Response;
        ...
        public async Task<ExecutionResponse<bool>> Delete(int id, RequestContext requestContext)
        {
            try
            {
                ...
                await context.Process(async () =>
                {
                    ...
                    var app = await dataHandler.PersonnelAccount.GetFirstAsync(x => x.Id == id);
                    var validationResponse = await this.PersonnelAccountValidator.ValidateAsync(app, ValidationMode.Deactivate);
                    ...
                    await dataHandler.PersonnelAccount.DeleteAsync(app);
                    await dataHandler.SaveAsync();
        ...
        public async Task<ExecutionResponse<bool>> IsUnique(PersonnelAccount Account, RequestContext requestContext)
        {
            try
            {
                var context = new ExecutionContext<bool>();
                await context.Process(async () =>
                {
                    ...
                    var isExisting = await dataHandler.PersonnelAccount.AnyAsync(x => (x.Name == Account.Name.Trim() ||
        ...
        public async Task<ExecutionResponse<bool>> IsExists(PersonnelAccountSearchCriteria criteria, RequestContext requestContext)
        {
            try
            {
                var context = new ExecutionContext<bool>();
                await context.Process(async () =>
                {
                    ...
                    var isExisting = await dataHandler.PersonnelAccount.AnyAsync(criteria);
                    return context.Response.Set(ResponseState.Success, isExisting);
        ...
        private bool Initialize()
        {
            ...
            isValid = isValid && (PersonnelAccountValidator != null);
        ...
        private bool MapUpdate(PersonnelAccount existing, PersonnelAccount updated, out List<int> addedSettings, out List<int> removedSettings)
        {
