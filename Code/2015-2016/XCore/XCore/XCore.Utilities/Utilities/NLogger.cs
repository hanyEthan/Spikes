    ...
    public static class NLogger
    {
        ...
        private static class Configuration
        {
            public static bool GetStatus()
            {
                try
                {
                    string destination = ConfigurationManager.AppSettings["NativeLogger.Location"];
                    if ( !string.IsNullOrEmpty( destination ) ) Settings.Target = destination;

                    string safeMode = ConfigurationManager.AppSettings["NativeLogger.SafeMode"];
                    if ( !string.IsNullOrEmpty( safeMode ) ) bool.TryParse( safeMode , out Settings.SafeMode );

                    string appSource = ConfigurationManager.AppSettings["NativeLogger.Application.Source"];
                    if ( !string.IsNullOrEmpty( appSource ) && string.IsNullOrEmpty( Settings.Source ) ) Settings.Source = appSource;

                    bool logToFile = false;
                    string fileLogging = ConfigurationManager.AppSettings["NativeLogger.FileSystemMode"];
                    ...
                    string sensitivityString = ConfigurationManager.AppSettings["NativeLogger.Sensitivity"];
                    ...
                    string autoCleanEnabled = ConfigurationManager.AppSettings["NativeLogger.AutoClean.Enabled"];
                    if ( !string.IsNullOrEmpty( autoCleanEnabled ) ) bool.TryParse( autoCleanEnabled , out Settings.LogCleaningEnabled );

                    string autoCleanAge = ConfigurationManager.AppSettings["NativeLogger.AutoClean.Age"];
        ...
        private class XLogEntry
        {
            ...
            public NLogger.Enums.LogSettings LogSettings { get; set; }
        ...
        //private class EnterpriseLibraryLog : ILogTarget
        //{
        //    #region Properties
        ...
        //    public void Log( XLogEntry entry , ILogFormatter formatter )
        //    {
        //        if ( !_Enabled || Settings.SafeMode ) return;

        //        try
        //        {
        //            var trace = TraceEventType.Information;
        //            switch ( entry.Status )
        //            {
        //                case Enums.LogStatus.Info: trace = TraceEventType.Information; break;
        //                case Enums.LogStatus.Error: trace = TraceEventType.Error; break;
        //                case Enums.LogStatus.Warning: trace = TraceEventType.Warning; break;
        //            }

        //            Logger.Write( CreateLogEntry( entry.Message , 0 , trace , entry.Context ) );
        //            var a = new EnterpriseLibraryLog();
        //        }
        //        catch ( Exception x )
        //        {
        //            _Enabled = false;
        //            Error( "NativeLogger ... Exception: " , x );
        //        }
        //    }
        //    public bool Clean( int logAge )
        //    {
        ...
        //    private static LogEntry CreateLogEntry( string message , int priority , TraceEventType severity , string context )
        //    {
        //        var extendedProperties = new Dictionary<string , object>();
        //        if ( severity == TraceEventType.Error )
        //        {
        //            extendedProperties.Add( "Stack" , GetContext() );
        //        }
        //        return new LogEntry( message , "General" , priority , 0 , severity , context , extendedProperties );
        //    }
        ...
        {
            return Trace( message , NLogger.Enums.LogSettings.Normal );
        }
        public static bool Trace( string message , params string[] parameters )
        {
            return Trace( HandleStringParameters( message , parameters ) );
        }
        public static bool Trace( string message , NLogger.Enums.LogSettings settings )
        {
            Log( new XLogEntry( DateTime.Now , message , "" , Enums.LogStatus.Trace , settings ) );
            return true;
        }

        public static bool Info( string message )
        {
            return Info( message , NLogger.Enums.LogSettings.Normal );
        }
        public static bool Info( string message , params string[] parameters )
        {
            return Info( HandleStringParameters( message , parameters ) );
        }
        public static bool Info( string message , NLogger.Enums.LogSettings settings )
        {
            Log( new XLogEntry( DateTime.Now , message , "" , Enums.LogStatus.Info , settings ) );
            return true;
        }
        ...
        public static bool Warning( string message , NLogger.Enums.LogSettings settings )
        {
            Log( new XLogEntry( DateTime.Now , message , "" , Enums.LogStatus.Warning , settings ) );
            return true;
        }

        public static bool Error( string message )
        {
            return Error( message , NLogger.Enums.LogSettings.Normal );
        }
        public static bool Error( string message , params string[] parameters )
        {
            return Error( HandleStringParameters( message , parameters ) );
        }
        public static bool Error( string message , NLogger.Enums.LogSettings settings )
        {
            Log( new XLogEntry( DateTime.Now , message , "" , Enums.LogStatus.Error , settings ) );
            return false;
        }
        ...
        public static bool Error( string message , Exception exception , string context )
        {
            try
            {
                ...
                Log( new XLogEntry( DateTime.Now , ( message ?? "" ) + exceptionString , context , Enums.LogStatus.Error ) );
                return false;
        ...
        private static bool Initialize()
        {
            ...
            {
                return Error( "NativeLogger ... Exception: " + x , Enums.LogSettings.TransientCaller );
            }
        }
        ...
        private static string GetContext( Enums.LogSettings logSettings )
        {
            ...
            if ( className.EndsWith( typeof( NLogger ).Name ) )
            {
        ...
        private static string HandleStringParameters( string message , string[] parameters )
        {
            ...
            {
                Error( "NativeLogger.HandleStringParameters ... Exception: " + x , Enums.LogSettings.TransientCaller );
            }
